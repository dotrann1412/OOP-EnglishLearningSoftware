...
VerticalScrollArea::VerticalScrollArea(int cols, QWidget* parent)
    ...
    contentWidget->setStyleSheet("QWidget{border-radius: 8px;}");
...
void VerticalScrollArea::addWidget(QWidget *w) {
    ...
    if(!(count % nCols)) {
        ...
        this->contentWidget->setFixedHeight(height + ConstantGUI::VERTICAL_SCROLL_AREA_PADDING * ((count + nCols - 1) / nCols + 1));
    }
